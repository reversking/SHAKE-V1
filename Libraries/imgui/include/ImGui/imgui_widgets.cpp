...
extern ImFont* PermanentMarker;
extern ImFont* RobotoBold;

namespace Images
{
    extern ID3D11ShaderResourceView* Logo;
    extern ID3D11ShaderResourceView* ArrowsIcon;
}
...
bool ImGui::DownloadNotification(const char* title, const char* description,
    const char* button1, const char* button2,
    const char* update_info, bool* p_open)
{
    ...
    state.time_elapsed += ImGui::GetIO().DeltaTime;
    state.alpha = ImClamp(state.time_elapsed * animation_speed, 0.0f, 1.0f);
    state.scale = ImLerp(0.95f, 1.0f, state.alpha);

    if (state.particles.size() < 50 && rand() % 10 == 0) {
        NotificationState::Particle p;
        p.pos = ImVec2(rand() % 400, rand() % 300);
        p.velocity = ImVec2((rand() % 100 - 50) * 0.02f, (rand() % 100 - 50) * 0.02f);
        p.size = 1.0f + (rand() % 10) * 0.2f;
    ...
    state.particles.erase(std::remove_if(state.particles.begin(), state.particles.end(),
        [](const NotificationState::Particle& p) { return p.life <= 0.0f; }), state.particles.end());
    ...
    const ImVec2 window_size = ImVec2(450, 350);
    ImGui::SetNextWindowPos(ImVec2(display_size.x * 0.5f, display_size.y * 0.5f),
        ImGuiCond_Always, ImVec2(0.5f, 0.5f));
    ImGui::SetNextWindowSize(window_size);
    ImGui::SetNextWindowBgAlpha(state.alpha);

    bool result = false;
    if (ImGui::Begin("DownloadNotification", p_open,
        ...
        ImGuiWindowFlags_NoScrollbar))
    {
        ...
        for (const auto& p : state.particles) {
            draw_list->AddCircleFilled(
                ImVec2(window_pos.x + p.pos.x, window_pos.y + p.pos.y),
                p.size * state.alpha,
                ImGui::GetColorU32(GuiSetting::BlackIn)
            );
        }

        ImVec2 title_size = ImGui::CalcTextSize(title);
        ImGui::SetCursorPos(ImVec2((window_size.x - title_size.x) * 0.5f, 25));
        ImGui::TextColored((GuiSetting::Black), title);

        draw_list->AddLine(
            ImVec2(window_pos.x + 30, window_pos.y + 70),
            ImVec2(window_pos.x + window_size.x - 30, window_pos.y + 70),
            ImGui::GetColorU32((GuiSetting::Black)), 1.5f
        );

        ImGui::PushFont(Roboto2);
        ImVec2 desc_size = ImGui::CalcTextSize("Are you sure you want to download this product?");
        ImGui::SetCursorPos(ImVec2((window_size.x - desc_size.x) * 0.5f, 100));
        ImGui::TextColored((GuiSetting::Black),
            "Are you sure you want to download this product?");

        ImVec2 text_size = ImGui::CalcTextSize(description);
        ImGui::SetCursorPos(ImVec2((window_size.x - text_size.x) * 0.5f, 140));
        ImGui::TextColored((GuiSetting::BlackIn), description);
        ImGui::PopFont();

        ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 8.0f);
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.2f, 0.2f, 0.2f, 0.8f * state.alpha));
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.3f, 0.3f, 0.3f, 0.9f * state.alpha));
        ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.15f, 0.15f, 0.15f, 0.8f * state.alpha));

        ImGui::SetCursorPos(ImVec2(80, window_size.y - 80));
        if (ImGui::Button(button1, ImVec2(120, 40))) {
            result = true;
            if (p_open) *p_open = false;
        }

        ImGui::SetCursorPos(ImVec2(window_size.x - 200, window_size.y - 80));
        if (ImGui::Button(button2, ImVec2(120, 40))) {
            if (p_open) *p_open = false;
        }
        ...
        ImGui::SetCursorPos(ImVec2((window_size.x - ImGui::CalcTextSize(update_info).x) * 0.5f, window_size.y - 30));
        ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, state.alpha * 0.7f), update_info);
    }
    ImGui::End();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImVec4 color, ImVec4 hover)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background,
        IsItemActive() || hovered ? GuiSetting::CheckboxBackground : GuiSetting::CheckboxBackground, g.IO.DeltaTime * 8.f);

    it_anim->second.text = ImLerp(it_anim->second.text,
        IsItemActive() || hovered ? GuiSetting::Black : GuiSetting::Black, g.IO.DeltaTime * 8.f);

    ImVec4 final_color = hovered ? hover : color;
    draw->AddRectFilled(bb.Min, bb.Max, GetColorU32(final_color), 4.0f);
...
bool ImGui::Button_Icon(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selectable ? GuiSetting::CheckboxBackground : hovered ? GuiSetting::ShadowTab : GuiSetting::BorderChild, g.IO.DeltaTime * 8.f);

    it_anim->second.text = ImLerp(it_anim->second.text, selectable || hovered ? GuiSetting::Black : GuiSetting::BlackIn, g.IO.DeltaTime * 8.f);

    it_anim->second.size = ImLerp(it_anim->second.size, -30.f, g.IO.DeltaTime * 3.f);

    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), 4.f);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2 + 5, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);

    GetWindowDrawList()->AddImage(image, ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2 - 19, bb.Min.y + 11), ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2 - 5, bb.Min.y + 25), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.text));

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}

bool ImGui::qrCode(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    GetWindowDrawList()->AddImage(image, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(GuiSetting::CheckboxBackground));

    GetWindowDrawList()->AddShadowRect(ImVec2(bb.Min.x, bb.Max.y - 20), ImVec2(bb.Max.x, bb.Max.y), ImGui::GetColorU32(GuiSetting::Shadow), 100.f, ImVec2(0, 0), ImDrawFlags_RoundCornersBottom);

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}
bool ImGui::Button(const char* label, const ImVec2& size_arg, ImVec4 color, ImVec4 hover)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, color, hover);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, GetColorU32(GuiSetting::ChildRect), window->WindowRounding, ImDrawFlags_RoundCornersBottomRight);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min + ImVec2(2, 0), grab_rect.Max, GetColorU32(GuiSetting::CheckboxBackground), style.ScrollbarRounding);
...
struct key_state
{
    ImVec4 background, text, rect;
    bool active = false;
    bool hovered = false;
    float alpha = 0.f, slow_show, line, line1;
...
bool ImGui::Keybind(const char* label, int* key, bool show_label)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float width = (GetContentRegionMax().x - 15 - style.WindowPadding.x);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - g.Style.WindowPadding.x) - square_sz;
    const ImRect rect(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(width, 30.f));
    const ImVec2 pos = window->DC.CursorPos;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(pos, pos + ImVec2(280, 25));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    const ImVec2 label_size1 = CalcTextSize(buf_display, NULL, true);

    ImRect clickable(ImVec2(rect.Max.x - 8 - label_size1.x, rect.Min.y), rect.Max + ImVec2(0, -4));
    bool hovered = ItemHoverable(total_bb, id);
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? GuiSetting::Black : GuiSetting::BlackIn, ImGui::GetIO().DeltaTime * 10.f);
    it_anim->second.slow_show = ImLerp(it_anim->second.slow_show, clickable.Min.x - rect.Min.x, g.IO.DeltaTime * 10.f);
    it_anim->second.rect = ImLerp(it_anim->second.rect, g.ActiveId == id ? GuiSetting::CheckboxBackground : GuiSetting::Separator, g.IO.DeltaTime * 10.f);
    window->DrawList->AddRectFilled(ImVec2(it_anim->second.slow_show + rect.Min.x - 1, clickable.Min.y - 3), clickable.Max - ImVec2(-1, 4), GetColorU32(GuiSetting::Color2));

    if (show_label) window->DrawList->AddText(rect.Min + ImVec2(0, 1), GetColorU32(GuiSetting::Black1), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y - 7), clickable.Max, buf_display, NULL, &label_size1, ImVec2(0.5f, 0.5f));
    PopStyleColor();
...
struct checkbox_state {
    ImVec4 background, text, circle, circle_outline;
    float move;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - 15 - g.Style.WindowPadding.x) - square_sz;

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2((w + square_sz), label_size.y));
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? GuiSetting::CheckboxBackground : GuiSetting::CheckboxInBackground, g.IO.DeltaTime * 10.f);
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? GuiSetting::CircleCheckbox : GuiSetting::CircleCheckboxIn, g.IO.DeltaTime * 10.f);

    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 14.f : 0.f, g.IO.DeltaTime * 12.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 8, 0), check_bb.Max + ImVec2(w, -6), GetColorU32(it_anim->second.background), 200.f);

    GetWindowDrawList()->AddCircleFilled(ImVec2(check_bb.Min.x + (w + it_anim->second.move), check_bb.Max.y - 15), 6.f, GetColorU32(it_anim->second.circle), 100.f);

    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y), GetColorU32(GuiSetting::Black), label);
...
struct DiamondState
{
    ImVec4 bgColor, image_color;
};

bool ImGui::DiamondButton(const char* label, ImTextureID image, ImTextureID image_bg, bool selected, const ImVec2& size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ItemSize(ImRect(bb.Min, bb.Max - ImVec2(0, 29)));
    if (!ItemAdd(bb, id)) return false;

    bool hovered, held, pressed = ButtonBehavior(bb, id, &hovered, &held);

    static std::map<ImGuiID, DiamondState> anim;
    auto& st = anim[id];

    ImVec4 target = selected || held ? GuiSetting::BorderChild : GuiSetting::CheckboxBackground;
    st.bgColor = ImLerp(st.bgColor, target, g.IO.DeltaTime * 12.0f);

    st.image_color = ImLerp(st.image_color, selected ? GuiSetting::Black : GuiSetting::Black, g.IO.DeltaTime * 12.0f);
    ...
    draw->AddImage(image_bg, ImVec2(points->x - 33, points->y + 6), ImVec2(points->x + 33, points->y + 72), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(st.bgColor));

    ImVec2 image_size = ImVec2(size.x * 0.5f, size.x * 0.5f);
    ImVec2 image_min = ImVec2(center.x - image_size.x * 0.5f, center.y - image_size.y * 0.5f);
    ImVec2 image_max = ImVec2(center.x + image_size.x * 0.5f, center.y + image_size.y * 0.5f);
    draw->AddImage(image, image_min + ImVec2(5, 5), image_max - ImVec2(5, 5), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(st.image_color));
...
struct tab_state
{
    ImVec4 lines, shadow, icon, text;
    float line, anim, size_shadow;
};

bool ImGui::Tab(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImRect image_bb(pos, pos + ImVec2(50, 50));

    ItemSize(size_arg, 0.f);
    if (!ItemAdd(bb, id)) return false;
    ...
    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? GuiSetting::Black : GuiSetting::BlackIn, g.IO.DeltaTime * 12.f);

    it_anim->second.anim = ImLerp(it_anim->second.anim, selectable ? 4.f : 0.f, g.IO.DeltaTime * 12.f);

    it_anim->second.lines = ImLerp(
        it_anim->second.lines,
        selectable ? ImVec4(GuiSetting::LineChild.x, GuiSetting::LineChild.y, GuiSetting::LineChild.z, GuiSetting::LineChild.w) : ImVec4(1.0f, 1.0f, 1.0f, 0.0f),
        g.IO.DeltaTime * 12.f
    );

    it_anim->second.line = ImLerp(it_anim->second.line, selectable ? size_arg.x / 2 : 0.f, g.IO.DeltaTime * 12.f);

    it_anim->second.shadow = ImLerp(
        it_anim->second.shadow,
        selectable ? ImVec4(GuiSetting::ShadowTab.x, GuiSetting::ShadowTab.y, GuiSetting::ShadowTab.z, GuiSetting::ShadowTab.w) : ImVec4(1.0f, 1.0f, 1.0f, 0.0f),
        g.IO.DeltaTime * 12.f
    );

    it_anim->second.size_shadow = ImLerp(it_anim->second.size_shadow, selectable ? 30.f : 0.f, g.IO.DeltaTime * 12.f);

    GetForegroundDrawList()->AddShadowCircle(bb.Min + ImVec2(20, 65), 20.f, GetColorU32(it_anim->second.shadow), 80.f, ImVec2(0, 0), 0);

    GetForegroundDrawList()->AddRectFilled(bb.Min + ImVec2(-it_anim->second.line + 15, size_arg.y - 3), bb.Max + ImVec2(it_anim->second.line - 15, 0), GetColorU32(it_anim->second.lines), 100.f, ImDrawFlags_RoundCornersBottom);

    GetForegroundDrawList()->AddImage(
        image,
        bb.Min + ImVec2(10, 5 - it_anim->second.anim),
        bb.Min + ImVec2(34, 29 - it_anim->second.anim),
        ImVec2(0, 0),
        ImVec2(1, 1),
        GetColorU32(it_anim->second.icon)
    );

    PushFont(Roboto2);
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Min.y + 30 + it_anim->second.anim), GetColorU32(it_anim->second.icon), label);
    PopFont();
...
struct Switch_state
{
    ImVec4 background, text, line;
};

bool ImGui::Switch(const char* label, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selectable ? GuiSetting::LineChild : GuiSetting::SwitchInActive, g.IO.DeltaTime * 8.f);

    it_anim->second.text = ImLerp(it_anim->second.text, selectable ? GuiSetting::SwitchText : GuiSetting::BlackIn, g.IO.DeltaTime * 8.f);
    it_anim->second.line = ImLerp(it_anim->second.line, selectable ? GuiSetting::LineChild : GuiSetting::BorderChild, g.IO.DeltaTime * 8.f);

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.background), 4.f);

    GetWindowDrawList()->AddRect(ImVec2(bb.Min.x - 1, bb.Min.y - 1), ImVec2(bb.Max.x + 1, bb.Max.y + 1), GetColorU32(it_anim->second.line), 4.f, 0, 1);

    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);

    return pressed;
}
bool ImGui::Switch_Tab(const char* label, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.background), 10.f);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);

    return pressed;
}
bool ImGui::Image_Icon(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? GuiSetting::Black : hovered ? GuiSetting::Black : GuiSetting::BlackIn, g.IO.DeltaTime * 12.f);
    ...
    GetForegroundDrawList()->AddImage(image, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));

    return pressed;
}
bool ImGui::Name_Category(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    ImVec2 location = ImVec2(bb.Min.x + 50, bb.Min.y - 15);
    ImVec2 size = ImVec2(45, 45);

    ImVec2 imageMin = ImVec2(location.x - size.x, location.y);
    ImVec2 imageMax = ImVec2(location.x, location.y + size.y);

    GetForegroundDrawList()->AddImage(Images::Logo, imageMin, imageMax);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x, bb.Max.y), GetColorU32(GuiSetting::Separator));
        if (g.LogEnabled)
            LogText(" |");
    }
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        if (item_visible)
        {
            ...
            window->DrawList->AddLine(bb.Min + ImVec2(0, 0), ImVec2(bb.Max.x - 15, bb.Min.y), GetColorU32(GuiSetting::Separator));
            if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const float arrow_size = 28;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : GetContentRegionMax().x - style.WindowPadding.x;

    ImRect bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w - 15, 30));
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, label_size.y + 30));
    ItemSize(ImRect(total_bb.Min, total_bb.Max - ImVec2(0, 20.f)));
    if (!ItemAdd(total_bb, id, &bb)) return false;

    ImRect bb_click_area = bb;
    bb_click_area.Min.y += 30;
    bb_click_area.Max.y += 30;
    bool hovered, held, pressed = ButtonBehavior(bb_click_area, id, &hovered, &held);
    ...
    window->DrawList->AddText(ImVec2(bb.Min.x, bb.Min.y), GetColorU32(GuiSetting::Black1), label);
    bb.Min.y += 30;
    bb.Max.y += 30;
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 30) + 7 : 0.f, g.IO.DeltaTime * 12.f);
    it_anim->second.arrow_roll = ImLerp(it_anim->second.arrow_roll, it_anim->second.opened_combo ? -1.f : 1.f, g.IO.DeltaTime * 12.f);
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? GuiSetting::Black : GuiSetting::BlackIn, g.IO.DeltaTime * 6.f);
    it_anim->second.icon = ImLerp(it_anim->second.icon, it_anim->second.opened_combo ? GuiSetting::TextActive : GuiSetting::TextActive, g.IO.DeltaTime * 6.f);
    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    window->DrawList->AddRect(ImVec2(bb.Min.x - 1, bb.Min.y - 3), ImVec2(bb.Max.x + 1, bb.Max.y), GetColorU32(GuiSetting::Color3), 5.f, 0, 3.f);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y - 2), ImVec2(bb.Max.x, bb.Max.y - 1), GetColorU32(GuiSetting::ChildRect), 5.f);

    GetWindowDrawList()->AddImage(Images::ArrowsIcon, ImVec2(value_x2 + 7, bb.Min.y + 10), bb.Max - ImVec2(8, 10), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.text));
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2(10, 5), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();
    ...
    ImGuiWindowFlags window_flags = ImDrawFlags_RoundCornersTop | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoScrollWithMouse;
...
bool ImGui::Combo(const char* label, int* currentItem, bool (*itemsGetter)(void*, int, const char**), void* data, int itemCount, int popupMaxHeightItems, bool multiSelect) {
    ImGuiContext& g = *GImGui;
    const char* previewValue = nullptr;
    static char multiPreview[512];
    multiPreview[0] = '\0';
    ...
    if (multiSelect) {
        ...
        for (int i = 0; i < itemCount; i++) {
            if (*currentItem & (1 << i)) {
                ...
                if (!itemsGetter(data, i, &itemText)) itemText = "*Unknown*";
                if (!first) strcat(multiPreview, ", ");
                strcat(multiPreview, itemText);
                first = false;
            }
        }
        previewValue = multiPreview[0] ? multiPreview : "None";
    }
    else {
    ...
    if (popupMaxHeightItems != -1 && !(g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint))
        SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popupMaxHeightItems)));

    if (!BeginCombo(label, previewValue, itemCount, ImGuiComboFlags_None))
        return false;
    ...
    for (int i = 0; i < itemCount; i++) {
        PushID(i);
        const char* itemText = nullptr;
        if (!itemsGetter(data, i, &itemText)) itemText = "*Unknown item*";

        bool isSelected = multiSelect ? ((*currentItem & (1 << i)) != 0) : (i == *currentItem);
        ImGuiSelectableFlags flags = multiSelect ? ImGuiSelectableFlags_DontClosePopups : ImGuiSelectableFlags_None;

        if (Selectable(itemText, isSelected, flags)) {
            valueChanged = true;
            if (multiSelect)
                *currentItem ^= (1 << i);
            else
        ...
        if (!multiSelect && isSelected) SetItemDefaultFocus();
        PopID();
    }
...
bool ImGui::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items, bool multi_select)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, multi_select);
    return value_changed;
}

bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items, bool multi_select)
{
    int items_count = 0;
    const char* p = items_separated_by_zeros;
    while (*p)
    ...
    bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items, multi_select);
    return value_changed;
}
...
struct slider_state {
    ImVec4 background, background1, background_rect, circle, circle_smile, text;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y), window->DC.CursorPos + ImVec2(w - 6, label_size.y + 17.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, 0.0f));

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, 26.f)));
    if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
        return false;
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? GuiSetting::Black : GuiSetting::BlackIn, g.IO.DeltaTime * 10);

    it_anim->second.circle = ImLerp(it_anim->second.circle, IsItemActive() ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 255), g.IO.DeltaTime * 10);

    it_anim->second.background1 = ImLerp(it_anim->second.background1, IsItemActive() ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 255), g.IO.DeltaTime * 10);

    it_anim->second.background_rect = ImLerp(it_anim->second.background_rect, IsItemActive() ? ImColor(33, 33, 33, 255) : ImColor(33, 33, 33, 255), g.IO.DeltaTime * 10);

    it_anim->second.circle_smile = ImLerp(it_anim->second.circle_smile, IsItemActive() ? ImColor(181, 176, 176, 255) : ImColor(181, 176, 176, 255), g.IO.DeltaTime * 10);
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 13), frame_bb.Max + ImVec2(-8, 1), GetColorU32(GuiSetting::CheckboxInBackground), 100.f);

    if (grab_bb.Max.x > grab_bb.Min.x) {

        it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 25.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 13), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y + 3), GetColorU32(GuiSetting::CheckboxBackground), 100.f, ImDrawFlags_RoundCornersAll);

        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y), 8.f, GetColorU32(GuiSetting::CircleCheckbox));

        GetWindowDrawList()->AddCircle(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y), 4.f, GetColorU32(GuiSetting::CheckboxBackground), 0, 3.f);

    }

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(frame_bb.Min - ImVec2(8, 22), frame_bb.Max - ImVec2(8, 14), value_buf, value_buf_end, NULL, ImVec2(1.0f, 1.0f));
    PopStyleColor();

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w + 5, frame_bb.Min.y - 16), GetColorU32(GuiSetting::Black1), label);
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = InputTextEx(label, NULL, NULL, buf, buf_size, bb.GetSize(), flags | ImGuiInputTextFlags_MergedItem);
    if (init)
    {
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText("", NULL, buf, IM_ARRAYSIZE(buf), flags))
    ...
    {
        if (InputText(label, NULL, buf, IM_ARRAYSIZE(buf), flags))
            value_changed = DataTypeApplyFromText(buf, data_type, p_data, format);
    }
    if (value_changed)
...
bool ImGui::InputText(const char* label, ImTextureID image, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline));
    return InputTextEx(label, image, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}

bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    return InputTextEx(label, NULL, NULL, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
}

bool ImGui::InputTextWithHint(const char* label, ImTextureID image, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline));
    return InputTextEx(label, image, hint, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}
...
struct inputtext_state {
    ImVec4 textActive = ImColor(255, 255, 255, 255);
    ImVec4 text_hov = ImColor(76, 75, 80, 255);
    ImVec4 text = ImColor(66, 65, 70, 255);
    ImVec4 line;
    ImVec4 text_color;
    ImVec4 rect_multi_1;
    ImVec4 rect_multi_2;
    ImVec2 slow_moveX0, slow_moveX1;
    float slow, outline_offset, outline_size;
    ImVec4 outline_color;
    float speed, value, line1;
};

bool ImGui::InputTextEx(const char* label, ImTextureID image, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    ImGui::SetCursorPos(GetCursorPos() + ImVec2(0, 0));
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 91, (is_multiline ? g.FontSize * 8.0f : label_size.y + 5) + style.FramePadding.y * 4.0f);
    ...
    const ImRect frame_bb1(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(280, 70));

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb1(pos, pos + ImVec2(280, 25));
    ...
    if (is_multiline)
    {
        ...
        ItemSize(total_bb, style.FramePadding.y);
        if (!ItemAdd(total_bb, id, &frame_bb1, ImGuiItemFlags_Inputable))
        {
            EndGroup();
            return false;
        }
        ...
        bool child_visible = BeginChildEx(label, NULL, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    const bool hovered1 = ItemHoverable(total_bb1, id);
    const bool hovered = ItemHoverable(frame_bb, id);
    if (hovered)
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, inputtext_state() });
        it_anim = anim.find(id);
    }
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? GuiSetting::Black : GuiSetting::BlackIn, g.IO.DeltaTime * 10.f);
    ...
    if (!is_multiline)
    {
        ...
        GetWindowDrawList()->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), GetColorU32(GuiSetting::Separator), 4.f);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(4, 4), rect.Max + ImVec2(14, 7), ImColor(30, 30, 30, 200), 4.f);
                }
        ...
        {
            ImU32 col = GetColorU32(it_anim->second.text);

            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(8, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                if ((frame_bb.Min.x + it_anim->second.value) < cursor_screen_rect.Min.x) {
                    it_anim->second.speed = (cursor_screen_rect.Min.x - (frame_bb.Min.x + it_anim->second.value)) / 12.f;
                    it_anim->second.value += it_anim->second.speed / ImGui::GetIO().Framerate * 200.f;
                }
                else if ((frame_bb.Min.x + it_anim->second.value) > cursor_screen_rect.Min.x) {
                    it_anim->second.speed = ((frame_bb.Min.x + it_anim->second.value - cursor_screen_rect.Min.x)) / 12.f;
                    it_anim->second.value -= it_anim->second.speed / ImGui::GetIO().Framerate * 200.f;
                }

                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 7, 10), frame_bb.Min + ImVec2(it_anim->second.value + 8, 24), GetColorU32(GuiSetting::Black));
            }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImU32 col = GetColorU32(it_anim->second.text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(8, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild("undopoints", NULL, ImVec2(0.0f, GetTextLineHeight() * 15), true))
    {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputText("##Text", NULL, buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* p = buf;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_Border, GetColorU32(ImGuiCol_WindowBg));

        PushStyleVar(ImGuiStyleVar_PopupRounding, 0);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 0), GetColorU32(GuiSetting::Black), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, 4, ImDrawCornerFlags_All);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, 0, ImDrawCornerFlags_All);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 0, ImDrawCornerFlags_All);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = 23, w = (GetContentRegionMax().x - 15 - g.Style.WindowPadding.x) - default_size;
    const ImVec2 size(size_arg.x == 0.0f ? default_size : size_arg.x, size_arg.y == 0.0f ? default_size : size_arg.y);
    const ImRect bb(window->DC.CursorPos + ImVec2(w - 4, 3), window->DC.CursorPos + size + ImVec2(w + 1, -4));
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 100.f, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
        ImVec4 col_source = (flags & ImGuiColorEditFlags_AlphaPreview) ? col_rgb : col_rgb_without_alpha;
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 100.f);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 100.f);

        window->DrawList->AddRect(bb_inner.Min - ImVec2(2, 2), bb_inner.Max + ImVec2(2, 2), GetColorU32(col_source), 100.f);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.color = ImLerp(it_anim->second.color, selected ? GuiSetting::Black : GuiSetting::BlackIn, g.IO.DeltaTime * 12.f);

    it_anim->second.move = ImLerp(it_anim->second.move, selected ? 10.f : 0.f, g.IO.DeltaTime * 12.f);

    it_anim->second.cirlce = ImLerp(it_anim->second.cirlce,
        selected ? GuiSetting::CheckboxBackground : ImVec4(0, 0, 0, 0),
        g.IO.DeltaTime * 12.f);
    ...
    window->DrawList->AddShadowCircle(ImVec2(bb.Min.x + 5 + it_anim->second.move, bb.Min.y + 15), 3.f, GetColorU32(it_anim->second.cirlce), 10.f, ImVec2(0, 0));

    window->DrawList->AddCircleFilled(ImVec2(bb.Min.x + 5 + it_anim->second.move, bb.Min.y + 15), 2.f, GetColorU32(it_anim->second.cirlce));

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.color));

    RenderTextClipped(text_min + ImVec2(8 + it_anim->second.move, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    BeginChildFrame(id, NULL, frame_bb.GetSize());
    return true;
}
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", NULL, NULL, NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
