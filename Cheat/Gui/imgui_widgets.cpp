...
#include <map>
#include "imgui.h"
#include <menu/helpers/elastic.h>
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size), inner_rect.Min.y + border_top + 16.f, outer_rect.Max.x - border_size, inner_rect.Max.y - border_size - 16.f);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    const float scrollbar_size_v = (axis == ImGuiAxis_X) ? bb.GetWidth() : (bb.GetHeight() + 2.f);
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) - 1.5f, bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    static std::map<ImGuiID, input_s> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, input_s() });
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRect(rect.Min, rect.Max, bg_color, 4.f);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            {
                ...
                draw_window->DrawList->AddLine(ImVec2(frame_bb.Min.x + it_anim->second.value, cursor_screen_rect.Min.y),
                    ImVec2(frame_bb.Min.x + it_anim->second.value, cursor_screen_rect.GetBL().y),
        ...
        {
            it_anim->second.speed = 0.f;
            it_anim->second.value = 0.f;
        }
    }
    else
    {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 picker_pos = window->DC.CursorPos + ImVec2(10.f, 10.f);
    ...
    float bar0_pos_x = picker_pos.x + sv_picker_size + style.ItemInnerSpacing.x + 5.f;
    ...
    if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        SameLine(); Dummy(ImVec2(1.f, 10.f));
        Dummy(ImVec2(10.f, 2.f));
    }
    ...
    if (sv_cursor_pos.x > 0.f)
    {
        static std::map<ImGuiID, elastic_xy> anim;
        ImGuiID hf = window->GetID(label);
        auto it_anim = anim.find(hf);

        if (it_anim == anim.end())
        {
            anim.insert({ hf, elastic_xy(sv_picker_size / 2.f, sv_picker_size / 2.f) });
            it_anim = anim.find(hf);
        }
        it_anim->second.x.set_target(sv_cursor_pos.x - picker_pos.x);
        it_anim->second.y.set_target(sv_cursor_pos.y - picker_pos.y);

        float sv_cursor_rad = ((!it_anim->second.x.is_at_rest() || !it_anim->second.y.is_at_rest()) || value_changed_sv) ? wheel_thickness * 0.55f : wheel_thickness * 0.40f;
        int sv_cursor_segments = draw_list->_CalcCircleAutoSegmentCount(sv_cursor_rad);
        ImVec2 pos_anim = picker_pos + ImVec2(it_anim->second.x.get_pos(), it_anim->second.y.get_pos());

        draw_list->AddCircleFilled(pos_anim, sv_cursor_rad, user_col32_striped_of_alpha, sv_cursor_segments);
        draw_list->AddCircle(pos_anim, sv_cursor_rad + 1, col_midgrey, sv_cursor_segments);
        draw_list->AddCircle(pos_anim, sv_cursor_rad, col_white, sv_cursor_segments);

        if (pos_anim.x < picker_pos.x)
        {
            it_anim->second.x.avoid_v();
        }
        else if (pos_anim.x > (picker_pos.x + sv_picker_size))
        {
            it_anim->second.x.avoid_v();
        }

        if (pos_anim.y < picker_pos.y)
        {
            it_anim->second.y.avoid_v();
        }
        else if (pos_anim.y > (picker_pos.y + sv_picker_size))
        {
            it_anim->second.y.avoid_v();
        }

        it_anim->second.x.update(0.16f);
        it_anim->second.y.update(0.16f);
    }
